...
@section('content')
    ...
    @endif
              <form method="post" action="{{ route('manage_feed.update',$feed->id)}}" enctype="multipart/form-data" class="form">
              @csrf
              @method('PATCH')
                <div class="card-body">
                    ...
                    <select name="site_id" class="form-control">
                      @if(!$sites->isEmpty())
                        @foreach($sites as $site)
                          <option value="{{$site->id}}" {{$site->id==$feed->site_id?'selected':''}}>{{$site->title}}</option>
                        @endforeach
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" name="url" value="{{ $feed->url }}"class="form-control" id="url" placeholder="Enter feed url">
                    @error('url')
                        <span class="text-danger" role="alert">
                            <strong>@lang(strtolower($message))</strong>
                        </span>
